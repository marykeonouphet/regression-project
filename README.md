...
## Odd observations
```{r odds}
proj2.data[c(which(proj2.data$sewer=="none")),]
proj2.data[c(which(proj2.data$bathrooms<1)),]
proj2.data[c(which(proj2.data$lotSize==0)),]
proj2.data[c(which(proj2.data$landValue > proj2.data$price)),]
```
...
plot(fitted.values(myfullmodel), t, xlab="fitted values", ylab="studentized residuals")
...
## sqrt(price) = 229.5 + 8.280lotSize - 0.287age + 0.0008674landValue + 0.0698livingArea + 24.72bathrooms + 2.171rooms + 13.08heatinghotair + 4.516heatinghotwatersteam + 115.8waterfrontYes - 41.30newConstructionYes + 10.77centralAirYes
